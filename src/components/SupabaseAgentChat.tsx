...
interface SupabaseAgentChatProps {
  agent: Agent;
  onBack: () => void;
}
...
export const SupabaseAgentChat: React.FC<SupabaseAgentChatProps> = ({ agent, onBack }) => {
  ...
  const loadChatHistory = async () => {
    try {
      ...
      const chatMessages: ChatMessage[] = history.map(msg => ({
        id: msg.id,
        content: msg.message_type === 'user' ? msg.message : msg.response || msg.message,
        sender: msg.message_type === 'user' ? 'user' : 'agent',
        timestamp: new Date(msg.created_at),
        type: msg.message_type === 'coordination' ? 'coordination' : 'text'
      }));
  ...
  const setupRealtimeSubscription = () => {
    ...
    messageSubscription.current = subscribeToMessages((payload) => {
      const newMessage = payload.new;
      if (newMessage.agent_id === agent.id) {
        const chatMessage: ChatMessage = {
          id: newMessage.id,
          content: newMessage.message_type === 'user' ? newMessage.message : newMessage.response || newMessage.message,
          sender: newMessage.message_type === 'user' ? 'user' : 'agent',
          timestamp: new Date(newMessage.created_at),
          type: newMessage.message_type === 'coordination' ? 'coordination' : 'text'
        };
  ...
  const handleSendMessage = async () => {
    if (!inputMessage.trim() || isTyping || !currentUser) return;
    ...
    try {
      ...
      if (response.coordination) {
        await handleAgentCoordination(agent.id, response.coordination);
      }
...
Respond naturally and in character.`;
  ...
  const handleAgentCoordination = async (fromAgent: string, coordination: { targetAgent: string; action: string; data?: any }) => {
    try {
      // Log coordination to database
      await logCoordination({
        from_agent: fromAgent,
        to_agent: coordination.targetAgent,
      ...
      await saveMessage({
        agent_id: fromAgent,
        message: coordinationMessage.content,
        message_type: 'coordination'
      });
  ...
  if (loading) {
    return (
      <div className="bg-gray-800 rounded-xl border border-gray-700 h-[600px] flex items-center justify-center">
